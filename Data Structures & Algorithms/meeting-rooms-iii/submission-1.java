class Solution {
    public int mostBooked(int n, int[][] meetings) {
        ...
        int [] count = new int [n] ;
        long [][] occupancy = new long [n][2] ;
        for(int i=0;i<meetings.length;i++)
        {
            pq.add(new long[]{meetings[i][0],meetings[i][1]});
        }
        while(!pq.isEmpty())
        {
            long [] node = pq.poll(); boolean isInsert = false ;
            for(int i=0;i<n;i++)
            {
